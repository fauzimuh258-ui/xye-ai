...
const MODES: { value: Mode; label: string; hint: string }[] = [
  ...
  { value: 'PROMPT_ENGINEER', label: 'Prompt', hint: 'Craft a prompt for another AI' },
];
    ...
    <div role="tablist" aria-label="Xye AI mode" className="flex flex-wrap gap-1.5">
      {MODES.map((m) => (
        <button
          ...
          title={m.hint}
          className={cn(
            'rounded-full px-3.5 py-1.5 text-xs font-medium transition-colors disabled:cursor-not-allowed disabled:opacity-50',
            value === m.value ? 'bg-emerald-500 text-zinc-950' : 'bg-zinc-800 text-zinc-400 hover:bg-zinc-700 hover:text-zinc-200'
          )}
        >
          {m.label}
        </button>
